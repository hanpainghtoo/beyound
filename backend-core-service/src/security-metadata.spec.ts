...
import { BillingAccountController } from './platform-admin/billing-account.controller';
...
import { PlatformMediaController } from './platform-admin/platform-media.controller';
...
describe('controller security metadata', () => {
  ...
  it('audits and protects billing account mutations on the platform-admin boundary', () => {
    const billingController = BillingAccountController.prototype;

    expect(
      Reflect.getMetadata('roles', billingController.getAllBillingAccounts),
    ).toEqual(
      expect.arrayContaining(['finance_viewer', 'support_viewer', 'read_only']),
    );
    expect(
      Reflect.getMetadata('roles', billingController.deleteBillingAccount),
    ).toEqual(['super_admin']);
    expect(auditFor(billingController, 'createBillingAccount')).toEqual({
      action: 'billing_account_created',
      resourceType: 'billing_account',
    });
    expect(auditFor(billingController, 'updateBillingAccount')).toEqual({
      action: 'billing_account_updated',
      resourceType: 'billing_account',
    });
  });

  it('protects platform media uploads with the dedicated platform-admin guard', () => {
    const guards = Reflect.getMetadata(
      GUARDS_METADATA,
      PlatformMediaController,
    );

    expect(guards).toEqual([PlatformAdminGuard]);
    expect(Reflect.getMetadata('path', PlatformMediaController)).toBe(
