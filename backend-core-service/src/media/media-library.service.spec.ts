...
import {
  MediaLibraryService,
  PLATFORM_BILLING_ACCOUNT_FILE_PURPOSE,
  PUBLIC_IMAGE_SIGNED_URL_TTL_SECONDS,
...
describe('MediaLibraryService storage limits', () => {
  ...
  it('creates platform uploads in the dedicated platform storage scope', async () => {
    const { service, request } = createService();

    await expect(
      service.createPlatformUpload('admin-1', {
        fileName: 'billing-logo.png',
        contentType: 'image/png',
        sizeBytes: 1024,
        purpose: 'billing-account-icon',
      }),
...
describe('MediaLibraryService public images', () => {
  const originalPublicFileStorageBaseUrl =
    process.env.PUBLIC_FILE_STORAGE_BASE_URL;
  ...
  beforeEach(() => {
    process.env.PUBLIC_FILE_STORAGE_BASE_URL = 'https://files.public.example';
  });

  afterAll(() => {
    if (originalPublicFileStorageBaseUrl === undefined) {
      delete process.env.PUBLIC_FILE_STORAGE_BASE_URL;
    } else {
      process.env.PUBLIC_FILE_STORAGE_BASE_URL = originalPublicFileStorageBaseUrl;
    }
    if (originalFileStoragePublicUrl === undefined) {
      delete process.env.FILE_STORAGE_PUBLIC_URL;
    } else {
      process.env.FILE_STORAGE_PUBLIC_URL = originalFileStoragePublicUrl;
    }
    if (originalFileStorageUrl === undefined) {
      delete process.env.FILE_STORAGE_URL;
    } else {
      process.env.FILE_STORAGE_URL = originalFileStorageUrl;
    }
  ...
  function createPublicImageService(file: Record<string, unknown>) {
    const { service, request } = createService();
    request.mockImplementation(async (path: string) => {
      if (!path.includes('download-url')) return file;
      return {
        file,
        download: { url: 'https://files.example/public-qr' },
      };
    });
    return { service, request };
  }

  it('returns a one-hour signed URL for a platform billing-account image', async () => {
    const file = {
      id: 'file-qr',
      tenantId: 'platform',
      purpose: PLATFORM_BILLING_ACCOUNT_FILE_PURPOSE,
      status: 'registered',
    };
    const { service, request } = createPublicImageService(file);

    await expect(service.getPublicImage('file-qr')).resolves.toMatchObject({
      download: { url: 'https://files.public.example/public-qr' },
    });
    expect(request).toHaveBeenNthCalledWith(
      2,
      `/files/file-qr/download-url?expiresInSeconds=${PUBLIC_IMAGE_SIGNED_URL_TTL_SECONDS}`,
      'platform',
    );
    expect(PUBLIC_IMAGE_SIGNED_URL_TTL_SECONDS).not.toBe(900);
  });

  it.each([
    { purpose: 'other-purpose', tenantId: 'platform' },
    {
      purpose: PLATFORM_BILLING_ACCOUNT_FILE_PURPOSE,
      tenantId: 'other-tenant',
    },
  ])('rejects files outside the public billing-account scope', async (file) => {
    const { service, request } = createPublicImageService({
      ...file,
      status: 'registered',
    });

    await expect(service.getPublicImage('file-qr')).resolves.toBeNull();
    expect(request).toHaveBeenCalledTimes(1);
  });

  it('uses the local file-storage address only as a development fallback', async () => {
    delete process.env.PUBLIC_FILE_STORAGE_BASE_URL;
    delete process.env.FILE_STORAGE_PUBLIC_URL;
    process.env.NODE_ENV = 'development';
    process.env.FILE_STORAGE_URL = 'http://127.0.0.1:6005';
    const file = {
      id: 'file-qr',
      tenantId: 'platform',
      purpose: PLATFORM_BILLING_ACCOUNT_FILE_PURPOSE,
    ...
    const { service } = createPublicImageService(file);

    await expect(service.getPublicImage('file-qr')).resolves.toMatchObject({
      download: { url: 'http://127.0.0.1:6005/public-qr' },
    });
  });
});
