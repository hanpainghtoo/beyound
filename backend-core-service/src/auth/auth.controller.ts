...
import { RegisterFromPricingDto } from './dto/register-from-pricing.dto';
...
export class AuthController {
  ...
  @ApiOperation({ summary: 'Register merchant from pricing page (creates account + billing)' })
  @ApiResponse({ status: 201, description: 'Registration initiated, OTP sent', type: AuthResponseDto })
  ...
  @Post('register/from-pricing')
  async registerFromPricing(
    @Body() registerDto: RegisterFromPricingDto,
    @Request() req,
  ) {
    return this.authService.registerFromPricing(registerDto, {
      ipAddress: req.ip || req.socket?.remoteAddress,
      userAgent: req.headers?.['user-agent'],
    });
  }

  @ApiOperation({ summary: 'Sign in with OTP (for new pricing registration)' })
  @ApiResponse({ status: 200, description: 'OTP verified, login successful', type: AuthResponseDto })
  ...
  @Post('otp-login')
  async otpLogin(@Body() body: { email: string; code: string }): Promise<AuthResponseDto> {
    return this.authService.otpLogin(body.email, body.code);
  }
