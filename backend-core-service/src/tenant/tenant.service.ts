...
export class TenantService {
      ...
      if (botId) {
        ...
        if (existing) {
          throw new ConflictException({
            ...
            message: 'A channel with this bot token already exists',
            existingChannelId: existing.id,
            existingChannelName: existing.displayName || existing.channelName,
          });
        }
      }
    }
  ...
  async validateTelegramToken(
    ...
    if (expectedUsername?.trim() && typeof username === 'string') {
      const expected = expectedUsername.trim().replace(/^@/, '').toLowerCase();
      const actual = username.trim().replace(/^@/, '').toLowerCase();
      if (expected !== actual) {
        return { ok: false, error: 'bot_username_mismatch' };
      }
    }
  ...
  /**
   * Best-effort provider-side webhook deregistration when a channel is
   * disconnected or deleted. Returns an error code on failure, null on
   * success or when there is nothing to deregister. Local state changes
   * always proceed regardless of the outcome.
   *
   * - telegram: Bot API deleteWebhook via integration-service with
   *   dropPendingUpdates=true so messages sent while disconnected are
   *   dropped instead of replayed on reconnect.
   * - viber: set_webhook with empty url via webhook-handler-service.
   * - messenger: shared app-level webhook; nothing per-channel to remove.
   * - tiktok: no provider webhook-delete API; the edge resolution gate
   *   (acknowledge_without_ingestion) drops leftover traffic.
   */
  private async cleanupProviderWebhookOnRemove(
    channel: TenantChannel,
  ): Promise<string | null> {
    try {
      ...
      if (channel.channelType === 'viber') {
        if (
          ...
          await this.callWebhookHandlerViberUnregister(channel.id, {
      ...
      return 'disconnect_cleanup_failed';
    }
  }

  private async callWebhookHandlerViberUnregister(
    channelId: string,
    body: Record<string, any>,
  ) {
    const webhookHandlerUrl = process.env.WEBHOOK_HANDLER_URL;
    if (!webhookHandlerUrl) {
      throw new Error('webhook_handler_unavailable');
    }
    const response = await fetch(
      `${webhookHandlerUrl.replace(/\/$/, '')}/webhooks/viber/${channelId}/register`,
      {
        method: 'DELETE',
        headers: {
    ...
    if (!response.ok) {
      throw new Error(`webhook_handler_http_${response.status}`);
    }
  }
  ...
  async disconnectChannel(
    ...
    const cleanupErrorCode = await this.cleanupProviderWebhookOnRemove(channel);
    if (cleanupErrorCode) {
      channel.webhookRegistrationStatus = 'failed';
      channel.webhookRegistrationErrorCode = cleanupErrorCode;
    }
  ...
  async deleteChannel(
    ...
    await this.cleanupProviderWebhookOnRemove(channel);
  ...
  private throwIfChannelDisabledForProviderTraffic(channel: TenantChannel) {
    if (
      channel.status === 'disabled' ||
      channel.status === 'inactive' ||
      channel.connectionStatus === 'disabled'
  ...
  async getInternalProviderVerification(channelId: string, provider: string) {
    ...
    this.throwIfChannelDisabledForProviderTraffic(channel);
  ...
  private async registerTelegramWebhook(
    ...
    const result = await this.callIntegrationTelegramWebhook('register', {
      ...
      // Drop anything queued while disconnected so gap messages are
      // missed rather than replayed on reconnect.
      dropPendingUpdates: true,
