...
export class AppService implements OnApplicationShutdown {
  ...
  getSignedDownload(tenantId: string, id: string, expiresInSeconds?: number) {
    const file = this.storage.get(id);
    if (!file || file.tenantId !== tenantId) {
      return { id, status: 'not_found' };
    }
    ...
    return {
      ...
      download: this.storage.createSignedDownloadUrl(file, {
        baseUrl: this.fileStoragePublicUrl(),
        expiresInSeconds: this.signedUrlTtlSeconds(expiresInSeconds),
      }),
    };
  }
  ...
  private signedUrlTtlSeconds(requestedTtl?: number) {
    const ttl =
      requestedTtl ?? Number(process.env.SIGNED_URL_TTL_SECONDS || 900);
