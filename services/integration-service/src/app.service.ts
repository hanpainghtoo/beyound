  ...
  async validateProvider(provider: string, input: ProviderCredentialInput) {
    ...
    if (definition.provider === 'telegram' && missingCredentials.length === 0) {
      try {
        ...
        const expectedUsername = this.stringValue(credentials.expectedUsername);
        if (expectedUsername) {
          const expected = expectedUsername.trim().replace(/^@/, '').toLowerCase();
          const actual = (identity.username || '').trim().replace(/^@/, '').toLowerCase();
          if (expected !== actual) {
            return {
              provider: definition.provider,
              ok: false,
              status: 'bot_username_mismatch',
              missingCredentials: [],
              requiredCredentials: definition.requiredCredentials,
              optionalCredentials: definition.optionalCredentials,
              errors: ['bot_username_mismatch'],
            };
          }
        }
        return {
