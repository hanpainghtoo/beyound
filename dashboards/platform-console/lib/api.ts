...
export type PlatformMediaFileDto = {
  id: string;
  tenantId: string;
  ownerId?: string;
  fileName: string;
...
export type PlatformMediaUploadDto = {
  file: PlatformMediaFileDto;
  upload: {
    driver: string;
    method: "PUT";
    url: string;
...
export type PlatformMediaDownloadDto = {
  file: PlatformMediaFileDto;
  download: {
    url: string;
    expiresAt?: string;
...
export async function uploadPlatformMedia(
  file: File,
  purpose = "platform-media",
) {
  const signed = await apiRequest<PlatformMediaUploadDto>(
    "/platform-admin/media/uploads",
    {
      method: "POST",
      body: JSON.stringify({
        fileName: file.name,
...
export async function getPlatformMediaDownloadUrl(fileId: string) {
  return apiRequest<PlatformMediaDownloadDto>(
    `/platform-admin/media/${encodeURIComponent(fileId)}/download-url`,
  );
}

export type BillingAccountType = "bank_account" | "digital_wallet";
...
export type UpdateBillingAccountInput = Partial<CreateBillingAccountInput>;

export type BillingAccountListOptions = {
  page?: number;
  limit?: number;
  search?: string;
  type?: BillingAccountType | "all";
  isPublic?: boolean | "all";
  includeInactive?: boolean;
};

export async function getPlatformBillingAccounts(
  options: BillingAccountListOptions = {},
) {
  ...
  if (options.isPublic === true) params.set("isPublic", "true");
  if (options.isPublic === false) params.set("isPublic", "false");
  if (options.includeInactive) params.set("includeInactive", "true");
  return apiRequest<PaginatedResult<BillingAccountDto>>(
    `/platform-admin/billing-accounts?${params.toString()}`,
  );
}

export function createPlatformBillingAccount(data: CreateBillingAccountInput) {
  return apiRequest<BillingAccountDto>("/platform-admin/billing-accounts", {
    method: "POST",
    body: JSON.stringify(data),
  });
}

export function updatePlatformBillingAccount(
  id: string,
  data: UpdateBillingAccountInput,
) {
  return apiRequest<BillingAccountDto>(
    `/platform-admin/billing-accounts/${id}`,
    {
      method: "PATCH",
      body: JSON.stringify(data),
    },
...
export function deactivatePlatformBillingAccount(id: string) {
...
export function activatePlatformBillingAccount(id: string) {
  return apiRequest<BillingAccountDto>(
    `/platform-admin/billing-accounts/${id}/activate`,
    { method: "POST" },
  );
}
