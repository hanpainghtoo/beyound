...
export default function MerchantsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [tenantResult, planResult, mockMerchantsResult] = await Promise.allSettled([
        getPlatformTenants(),
        getSubscriptionPlans(),
        fetch("http://localhost:6100/api/mock-merchants").then((r) => r.json()),
      ])

      const realTenants = tenantResult.status === "fulfilled" ? tenantResult.value.data : []
      const mockMerchants = mockMerchantsResult.status === "fulfilled" && mockMerchantsResult.value.success
        ? mockMerchantsResult.value.data.map((m: PlatformTenantDto & { _selfRegistered?: boolean; _paymentId?: string }) => ({
            ...m,
            _selfRegistered: m._selfRegistered,
            _paymentId: m._paymentId,
          }))
        : []

      setMerchants([...realTenants, ...mockMerchants])
      setPlans(planResult.status === "fulfilled" ? planResult.value : [])

      if (tenantResult.status === "rejected") {
        if (tenantResult.reason instanceof PlatformApiError && tenantResult.reason.status === 403) setPermissionDenied(true)
        setError(friendlyError(tenantResult.reason))
      }
    } catch (requestError) {
