...
import {
  ...
  Search,
  ShieldCheck,
  // Trash2, // hidden with delete buttons — restore to re-enable delete
  X,
} from "lucide-react";
...
import { useSubscriptionGate, billingKeys } from "@/lib/queries/billing";
...
export default function ChannelsPage() {
  ...
  const [pendingChannelIds, setPendingChannelIds] = useState<Set<string>>(
    () => new Set(),
  );
  ...
  const [pendingDuplicateConfirm, setPendingDuplicateConfirm] = useState<{
    ...
    payload: CreateTenantChannelInput;
  } | null>(null);
  ...
  const loadChannels = useCallback(async () => {
    ...
    try {
      ...
      void queryClient.invalidateQueries({ queryKey: billingKeys.periods() });
      void queryClient.invalidateQueries({ queryKey: billingKeys.usageSummary() });
    } catch {
      toast.error("Couldn't load your channels. Please try again.");
    } finally {
      setIsLoading(false);
    }
  }, [queryClient]);
    ...
    const interval = window.setInterval(async () => {
      try {
        ...
        if (request.status === "connected") {
          await loadChannels();
          toast.success("Your Telegram bot is connected and ready to use.");
        }
      } catch {
        toast.error(
          "Couldn't check bot status. Retrying...",
        );
      }
    }, 4000);
  ...
  useEffect(() => {
    ...
    if (connect === "success") {
      ...
      toast.success(`${pageName} is now connected.`);
    } else if (connect === "error") {
      toast.error(
        searchParams.get("message") ||
          "Messenger setup didn't complete. Please try connecting again.",
      );
    }
  }, [searchParams]);

  const withPendingChannel = useCallback(
    async (channelId: string, fn: () => Promise<void>) => {
  ...
  const createTelegramChannel = async () => {
    if (!telegramForm.channelName.trim()) {
      toast.error("Please enter your Bot username to continue.");
      return;
    }
    if (!telegramForm.botToken.trim()) {
      toast.error("Please enter your Telegram bot token to continue.");
      return;
    }
    ...
    try {
      ...
      if (tested.ok) {
        ...
        toast.error(tested.errors.join(" ") || "Bot verification failed. Please check your token and try again.");
      }
      void queryClient.invalidateQueries({ queryKey: billingKeys.periods() });
      void queryClient.invalidateQueries({ queryKey: billingKeys.usageSummary() });
    } catch (error) {
      if (
        ...
        error.status === 409 &&
        error.code === "TELEGRAM_BOT_ALREADY_CONNECTED"
      ) {
        const data = error.data as Record<string, unknown> | undefined;
        const existingChannelId = (data?.existingChannelId as string) || "";
        const existingChannelName =
          (data?.existingChannelName as string) || "this channel";
        if (existingChannelId) {
          setPendingDuplicateConfirm({
            existingChannelId,
            existingChannelName,
            payload: buildTelegramPayload(telegramForm),
          });
        } else {
          toast.error("This bot is already connected to another channel.");
        }
      } else {
        toast.error("Couldn't connect Telegram. Please try again.");
      }
    } finally {
  ...
  const validateTelegramToken = async () => {
    ...
    const expectedUsername = telegramForm.channelName.trim().replace(/^@/, "");
    if (!expectedUsername) {
      toast.error("Please enter your Bot username to verify.");
      return;
    }
    if (!token) {
      toast.error("Please enter a bot token to verify.");
      return;
    }
    ...
    try {
      const result = await tenantChannelsApi.validateTelegramToken(
        token,
        expectedUsername,
      );
      if (result.ok && result.username) {
        const actual = result.username.trim().replace(/^@/, "").toLowerCase();
        if (actual !== expectedUsername.toLowerCase()) {
          setValidatedBot(null);
          toast.error("Bot username does not match this token.");
          return;
        }
      }
      ...
      if (result.ok) {
        toast.success(
          result.firstName
            ? `Bot verified: ${result.firstName}${result.username ? ` (@${result.username})` : ""}`
            : "Bot token is valid.",
        );
      } else {
        toast.error(result.error || "That token didn't work. Please check it and try again.");
      }
    } catch {
      toast.error("Couldn't verify the token. Please try again.");
    } finally {
  ...
  const initiateTelegramManagedBot = async () => {
    ...
    if (!displayName || !suggestedUsername) {
      toast.error("Please fill in the bot name and username.");
      return;
    }
    ...
    try {
      ...
      toast.success("Opening Telegram — follow the steps there to finish setup.");
    } catch {
      toast.error(
        "Couldn't start bot creation. Please try again.",
      );
    } finally {
      setIsSaving(false);
    }
  ...
  const cancelTelegramManagedBot = async () => {
    ...
    try {
      ...
      setTelegramManagedRequest(request);
      toast.success("Bot setup was cancelled.");
    } catch {
      toast.error(
  ...
  const acknowledgeTelegramManagedBot = useCallback(() => {
    setTelegramManagedRequest(null);
    setTelegramManagedBotForm(defaultTelegramManagedBotForm);
    void loadChannels();
  }, [loadChannels]);

  const createViberChannel = async () => {
    if (!viberForm.authToken.trim()) {
      toast.error("Please enter your Viber auth token to continue.");
      return;
    }
    ...
    try {
      ...
      if (tested.ok) {
        toast.success("Viber channel connected successfully.");
      } else {
        toast.error(tested.errors.join(" ") || "Viber verification failed. Please check your token and try again.");
      }
      void queryClient.invalidateQueries({ queryKey: billingKeys.periods() });
      void queryClient.invalidateQueries({ queryKey: billingKeys.usageSummary() });
    } catch {
      toast.error("Couldn't connect Viber. Please try again.");
    } finally {
      setIsSaving(false);
    }
  };
  const testChannel = async (channel: TenantChannelDto) => {
    await withPendingChannel(channel.id, async () => {
      try {
      ...
      if (result.ok) {
        toast.success(`${providerLabel(result.provider as ConnectProvider)} is working correctly.`);
      } else {
        toast.error(result.errors.join(" ") || "Connection check failed. Please verify your settings and try again.");
      }
      void queryClient.invalidateQueries({ queryKey: billingKeys.periods() });
      void queryClient.invalidateQueries({ queryKey: billingKeys.usageSummary() });
    } catch {
      toast.error("Couldn't run connection check. Please try again.");
    }
    });
  };
  ...
  const saveChannelName = async (channel: TenantChannelDto) => {
    ...
    try {
      ...
      setEditingChannelId(null);
      toast.success(`Channel renamed to ${displayName}.`);
    } catch {
      toast.error("Couldn't rename. Please try again.");
    }
  };

  const disconnectChannel = (channel: TenantChannelDto) => {
    setPendingConfirm({ action: "disconnect", channel });
  };

  const deleteChannel = (channel: TenantChannelDto) => {
    setPendingConfirm({ action: "delete", channel });
  };

  const executeDisconnectChannel = async (channel: TenantChannelDto) => {
    await withPendingChannel(channel.id, async () => {
    try {
      ...
      toast.success(`${channelTitle(channel)} disconnected.`);
      void queryClient.invalidateQueries({ queryKey: billingKeys.periods() });
      void queryClient.invalidateQueries({ queryKey: billingKeys.usageSummary() });
    } catch {
  ...
  const executeDeleteChannel = async (channel: TenantChannelDto) => {
    await withPendingChannel(channel.id, async () => {
    try {
      ...
      toast.success(`${channelTitle(channel)} permanently deleted.`);
      void queryClient.invalidateQueries({ queryKey: billingKeys.periods() });
      void queryClient.invalidateQueries({ queryKey: billingKeys.usageSummary() });
    } catch {
  ...
  const executeConfirmDuplicateUpdate = async () => {
    if (!pendingDuplicateConfirm) return;
    const { existingChannelId, payload } = pendingDuplicateConfirm;
    setPendingDuplicateConfirm(null);
    setIsSaving(true);
    try {
      await tenantChannelsApi.update(existingChannelId, payload);
      const tested = await tenantChannelsApi.testConnection(existingChannelId);
      setChannels((current) => {
        const withoutUpdated = current.filter(
          (item) => item.id !== tested.channel.id,
        );
      ...
      setTelegramForm(defaultTelegramForm);
      if (tested.ok) {
        toast.success("Channel updated and connected successfully.");
      } else {
        toast.error(tested.errors.join(" ") || "Channel updated but verification failed. Please try again.");
      }
      void queryClient.invalidateQueries({ queryKey: billingKeys.periods() });
      void queryClient.invalidateQueries({ queryKey: billingKeys.usageSummary() });
    } catch {
      toast.error("Couldn't update channel. Please try again.");
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const setRetention = async (channel: TenantChannelDto, selected: boolean) => {
    try {
      ...
      toast.success(
        selected
          ? `${channelTitle(channel)} will be kept when your plan renews.`
          : `${channelTitle(channel)} will follow your plan's default on renewal.`,
      );
    } catch {
      toast.error(
        "Couldn't update retention setting. Please try again.",
      );
    }
  };
  ...
  const reactivateChannel = async (channel: TenantChannelDto) => {
    ...
    try {
      ...
      toast.success(`${channelTitle(channel)} is active again.`);
    } catch {
      toast.error("Couldn't reactivate. Please try again.");
    }
    });
  };
    ...
    <>
      ...
      >
        {activeDialog ? (
          <ConnectDialog
            ...
            onTelegramManagedOkay={acknowledgeTelegramManagedBot}
      ...
      >
        {detailsChannel ? (
          <ChannelDetailsDialog
            ...
            isPending={pendingChannelIds.has(detailsChannel.id)}
            onVerify={() => void testChannel(detailsChannel)}
            onDisconnect={() => disconnectChannel(detailsChannel)}
            onDelete={() => deleteChannel(detailsChannel)}
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {pendingConfirm?.action === "disconnect"
                ? "Disconnect Channel"
                : "Delete Channel"}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {pendingConfirm?.action === "disconnect"
                ? `Disconnect ${pendingConfirm ? channelTitle(pendingConfirm.channel) : ""}? The saved record will remain available.`
                : `Delete ${pendingConfirm ? channelTitle(pendingConfirm.channel) : ""} permanently? This removes the saved record and channel data.`}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={() => {
                ...
                if (pendingConfirm.action === "disconnect") {
                  void executeDisconnectChannel(pendingConfirm.channel);
                } else {
                  void executeDeleteChannel(pendingConfirm.channel);
                }
                setPendingConfirm(null);
              }}
            >
              {pendingConfirm?.action === "disconnect" ? "Disconnect" : "Delete"}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
      ...
      <AlertDialog
        ...
        onOpenChange={(open) => {
          if (!open) setPendingDuplicateConfirm(null);
        }}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Bot Already Connected</AlertDialogTitle>
            <AlertDialogDescription>
              This bot is already connected as{" "}
              <strong>{pendingDuplicateConfirm?.existingChannelName}</strong>.
          ...
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction
              onClick={() => void executeConfirmDuplicateUpdate()}
            >
              Update
            </AlertDialogAction>
          </AlertDialogFooter>
          ...
          {isLoading ? (
            ...
            <div className="divide-y divide-slate-200 dark:divide-slate-800">
              {filteredChannels.map((channel) => {
                ...
                const isChannelPending = pendingChannelIds.has(channel.id);
                return (
                  <div
                    key={channel.id}
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      {isChannelPending ? (
                        <span
                          aria-label="Channel action in progress"
                      ...
                      <DropdownMenu>
                        ...
                        >
                          ...
                          <DropdownMenuItem
                            disabled={
                              badgeState === "disconnected" || isChannelPending
                            }
...
function ChannelDetailsDialog({
  ...
  onVerify,
  onDisconnect,
  // onDelete, // hidden with delete buttons — restore to re-enable delete
  onRetention,
  onReactivate,
}: {
  channel: TenantChannelDto;
    ...
    <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-3xl">
      ...
      <div className="flex flex-wrap justify-end gap-2 border-t pt-4">
        ...
        <Button
          variant="outline"
          onClick={onVerify}
          disabled={channel.status === "disabled" || isPending}
        >
          {isPending ? (
            <Loader2 className="mr-2 h-4 w-4 animate-spin" />
          ) : (
          ...
          {channel.status === "inactive" ? "Reconnect" : "Verify"}
        </Button>
        <Button
          variant="outline"
          disabled={
    ...
    <DialogContent className="max-h-[92vh] overflow-y-auto border-slate-200 bg-white p-0 sm:max-w-5xl dark:border-slate-800 dark:bg-slate-950">
        ...
        <div className="space-y-5">
          {flow.provider === "telegram" ? (
            <TelegramConnectForm
              ...
              onManagedOkay={onTelegramManagedOkay}
              isSaving={isSaving}
              isConnectingTelegram={isConnectingTelegram}
      ...
      <div className="flex justify-end border-t border-slate-200 px-6 py-4 dark:border-slate-800">
        <Button
          variant="outline"
          onClick={onClose}
          disabled={isSaving || isValidating || isConnectingTelegram}
        >
          Close
        </Button>
      </div>
...
function TelegramConnectForm({
  ...
  const isManagedConnected = managedRequest?.status === "connected";
  const connectedBotUsername =
    managedRequest?.createdBotUsername ||
    managedRequest?.connectedChannel?.username ||
    "";
  const connectedBotName =
    managedRequest?.connectedChannel?.displayName ||
    managedForm.displayName ||
    "Telegram bot";

  return (
    <Tabs defaultValue="managed" className="w-full">
      <TabsList className="w-fit grid-cols-2">
        <TabsTrigger value="managed">Create business bot</TabsTrigger>
        <TabsTrigger value="existing">Connect existing bot</TabsTrigger>
      </TabsList>
        ...
        <div className="space-y-5">
          <div className="rounded-md border border-cyan-200 bg-cyan-50 p-4 text-sm text-cyan-950 dark:border-cyan-500/20 dark:bg-cyan-500/10 dark:text-cyan-100">
            <div className="flex gap-3">
              ...
              <p>
                This creates a Telegram bot owned by your business. Customers
                will message your business bot directly. The manager bot is
          ...
          <div className="grid gap-4 rounded-md border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-950 md:grid-cols-2">
            <div className="md:col-span-2">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                >
                  {managedRequest
                    ? formatStatus(managedRequest.status)
                    : "Ready"}
                </Badge>
              </div>
            </div>
                  ...
                  <div>
                    ...
                    {connectedBotUsername ? (
                      <a
                        ...
                        href={`https://t.me/${connectedBotUsername.replace(/^@/, "")}`}
                        target="_blank"
                        rel="noreferrer"
                      >
                        <Link2 className="mr-1.5 h-4 w-4" />@
                        {connectedBotUsername.replace(/^@/, "")}
                      </a>
                    ) : null}
                  </div>
                  <Badge
            ...
            {managedRequest && !isManagedConnected ? (
              <div className="md:col-span-2 rounded-md border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600 dark:border-slate-800 dark:bg-slate-900/70 dark:text-slate-300">
                <p className="flex items-center gap-2 font-medium text-slate-950 dark:text-slate-50">
                  {!terminalRequest ? (
            ...
            <div className="flex flex-wrap gap-2 md:col-span-2">
              {isManagedConnected ? (
                <Button onClick={onManagedOkay}>Okay</Button>
              ) : (
                <>
                  <Button
        ...
        <div className="space-y-5">
          <div className="rounded-md border border-cyan-200 bg-cyan-50 p-4 text-sm text-cyan-950 dark:border-cyan-500/20 dark:bg-cyan-500/10 dark:text-cyan-100">
            <div className="flex gap-3">
              <Bot className="mt-0.5 h-4 w-4 shrink-0" />
              <p>
                Already have a bot from BotFather? Enter its username and
                token, validate that the username matches the token, then
                connect.
              </p>
            </div>
          </div>
          ...
          <div className="grid gap-4 rounded-md border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-950 md:grid-cols-2">
            ...
            <div className="flex flex-wrap gap-2 md:col-span-2">
              <Button
                variant="outline"
                onClick={onValidate}
                disabled={isValidating || isConnectingTelegram}
              >
                {isValidating ? (
                  <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                ) : (
                ...
                {isValidating ? "Validating..." : "Validate bot"}
              </Button>
              {validatedBot?.ok ? (
                <Button
                  onClick={onSubmit}
