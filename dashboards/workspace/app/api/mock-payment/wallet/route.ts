...
export async function POST(request: Request) {
  try {
    ...
    const { paymentId, selectedOptionId, accountName, phoneNumber } = body
    ...
    payments[index].payment = {
      ...
      walletAccount: { accountName, phoneNumber },
    }
    ...
    return NextResponse.json({
      success: true,
      paymentId,
      message: "Digital wallet selected successfully",
    }, { headers: CORS_HEADERS })
  } catch (error) {
    console.error("[MockPayment Wallet] Error:", error)
    return NextResponse.json({ error: "Failed to select digital wallet" }, { status: 500, headers: CORS_HEADERS })
  }
}
